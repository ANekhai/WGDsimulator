...
class SimParameters:
    def __init__(self, wgd_type=None, num_genes=0, num_chr=0, del_p=[0, 0], ins_p=[0, 0], indel_length=[0, 0], duplication_p=[0, 0],
                 duplication_length=[0, 0], events=[0, 0], inter_p=0):
        ...
        self.inter_dup_p = inter_p

        assert np.isclose(self.rearrangement_p[0] + self.insertion_p[0] + self.deletion_p[0] + self.duplication_p[0], 1) and \
               np.isclose(self.rearrangement_p[1] + self.insertion_p[1] + self.deletion_p[1] + self.duplication_p[1], 1)
...
class Simulation:
    ...
    @staticmethod
    def apply_random_segmental_duplication(genome, duplication_length_range, current_copy_number, inter_p):
        chromosome = np.random.choice(genome.chromosomes)
        ...
        chromosome.copy_number[bp:bp+length] = copy_number_block

        #check if duplication happens to new chromosome
        if random.uniform(0, 1) < inter_p and len(genome.chromosomes) > 1:
    ...
    def apply_random_events(param, genome, current_insertion_gene, current_copy_number=None, postwgd=1):
        ...
        for event in events:  # number of events, can be weighted by 'scaling' parameters
            if event == EventType.REARRANGEMENT:
                ...
                Simulation.apply_random_segmental_duplication(genome, duplication_length_range, current_copy_number, param.inter_dup_p)
    ...
    #TODO: WGD events should update copy_number in genome class, they currently do not.
    @staticmethod
...
if __name__ == '__main__':
    ...
    parser.add_argument("-idup_p", "--inter_duplication_p", type=float, default=0.0, help="Percentage of segmental duplications that occur across two chromosomes")
    ...
    sim_par = SimParameters(wgd_type=param.WGD_type, num_genes=param.num_genes, num_chr=param.num_chr,
                            ...
                            duplication_length=[param.duplication_length, param.duplication_length2], events=[param.num_ev, param.num_ev2], inter_p=param.inter_duplication_p)
    # start sim object;
